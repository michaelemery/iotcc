...
def get_hub(mac):
    # get json object represent hub data
    url = SERVER + "/gethub"
    params = {"mac":str(mac)}
    resp = requests.get(url=url, params=params)
    hub = json.loads(resp.text)
    # populate the table with the json object.
    if not any(hub):
    	print ("Failed to load any hub data for MAC:" + mac + " Device was not configured.")
    	exit()
    return hub   
...
def show_config(config):
    ...
    for sensor in config['profile_sensor']:
        ...
        print("---")
        a = config['sensor_type']
        b = sensor['sensor_type_id']
        c = 'sensor_type_name'
        print("---")       
        print(a)
        print(b)
